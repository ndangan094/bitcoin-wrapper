...
class TronJS {
    ...
    async getInfoTRC20(rpc, address,privateKey) {
        ...
        return { symbol: result[0], decimals: result[1].toString() };
    }

    async getTRC20Balance( rpc,tokenContract, from, privateKey ) {
        const TronWeb = require('tronweb')
        const HttpProvider = TronWeb.providers.HttpProvider;
        const fullNode = new HttpProvider(rpc);
        const solidityNode = new HttpProvider(rpc);
    ...
    async transferTRC20(rpc,tokenContract, to, privateKey, amount) {
        const TronWeb = require('tronweb')
        ...
        const resp = await contract.methods.transfer(to, BigNumber.from(amount.toString())).send();
        while(true){
            try {
